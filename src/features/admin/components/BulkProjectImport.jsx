...
const BulkProjectImport = ({ onClose, onSuccess }) => {
    ...
    const validateData = (data) => {
        ...
        // Check columns
        const requiredCols = ['name', 'clientfirstname', 'clientlastname', 'clientemail', 'clientphone'];
        ...
        data.forEach((row, index) => {
            // Field mapping (handle case insensitivity)
            const mappedRow = {
                name: row.name || row.Name,
                clientFirstName: row.clientFirstName || row.clientfirstname || row['Client First Name'],
                clientLastName: row.clientLastName || row.clientlastname || row['Client Last Name'],
                clientEmail: row.clientEmail || row.clientemail || row['Client Email'],
                clientPhone: row.clientPhone || row.clientphone || row['Client Phone'],
                location: row.location || row.Location,
                budget: row.budget || row.Budget,
                propertyType: row.propertyType || row.propertytype || row['Property Type'],
                scopeOfWork: row.scopeOfWork || row.scopeofwork || row['Scope of Work'],
                cpNumber: row.cpNumber || row.cpnumber || row['CP Number']
            };

            if (!mappedRow.clientEmail || !mappedRow.name || !mappedRow.clientFirstName || !mappedRow.clientPhone) {
                errors[index] = "Missing required fields";
            } else if (!/\S+@\S+\.\S+/.test(mappedRow.clientEmail)) {
    ...
    const downloadTemplate = () => {
        const csv = "name,clientFirstName,clientLastName,clientEmail,clientPhone,location,budget,propertyType,scopeOfWork,cpNumber\nModern Villa,John,Doe,john@example.com,9876543210,Bangalore,5000000,Residential (Villa),Full Interior,CP001";
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
            ...
            >
                ...
                <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                    <div>
                        <h2 className="text-lg font-bold text-slate-800">Bulk Import Projects</h2>
                        <p className="text-xs text-slate-500 mt-1">Add multiple projects via CSV file</p>
                    </div>
                    ...
                    {step === 'upload' && (
                        <div className="space-y-6 text-center py-8">
                            ...
                            <button onClick={downloadTemplate} className="text-sm text-blue-600 font-medium hover:underline flex items-center justify-center gap-2">
                                <Download size={14} /> Download Project CSV Template
                            </button>
                                        ...
                                        {previewData.map((row, idx) => {
                                            ...
                                            return (
                                                <tr key={idx} className={error ? "bg-red-50/50" : "hover:bg-slate-50"}>
                                                    ...
                                                    <td className="px-4 py-3 text-slate-500">{row.clientFirstName} {row.clientLastName}</td>
                                                    <td className="px-4 py-3 text-slate-500">{row.clientEmail}</td>
                                                </tr>
                                            );
                                        })}
                                    </tbody>
                        ...
                        <div className="text-center py-8">
                            ...
                            <div className="grid grid-cols-2 gap-4 mt-8 mb-8">
                                <div className="bg-green-50 p-4 rounded-xl border border-green-100">
                                    ...
                                    <div className="text-xs text-green-700 font-medium uppercase tracking-wider">Created</div>
                                </div>
