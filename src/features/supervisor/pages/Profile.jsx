    ...
    return (
        <div className="max-w-2xl mx-auto space-y-6">
            ...
            <div className="bg-white rounded-3xl p-8 border border-slate-100 shadow-sm text-center">
                <div className="w-24 h-24 bg-slate-900 text-white text-3xl font-black rounded-full flex items-center justify-center mx-auto mb-6 shadow-xl ring-8 ring-slate-50">
                    {user.name?.charAt(0) || 'U'}
                </div>
                <h2 className="text-xl font-bold text-slate-900">{user.name}</h2>
                <p className="text-slate-500 font-medium">{user.department || 'Operations'}</p>
                <div className="mt-2 inline-block px-3 py-1 bg-indigo-50 text-indigo-600 rounded-full text-xs font-black uppercase tracking-wider">
                    {user.role}
                </div>
            </div>
